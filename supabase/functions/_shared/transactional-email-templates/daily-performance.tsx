    ...
    <Html lang="pt-BR" dir="ltr">
      ...
      <Body style={main}>
        <Container style={container}>
          ...
          <Heading style={h1}>Performance do dia</Heading>
          ...
          <Section style={totalsCard}>
            ...
            <Section style={totalsGrid}>
              <Section style={totalItem}>
                <Text style={totalNumber}>{totalStageChanges}</Text>
                <Text style={totalLabel}>↗️ Avanços</Text>
              </Section>
              <Section style={totalItem}>
                <Text style={totalNumber}>{totalAppointments}</Text>
                <Text style={totalLabel}>📅 Agend.</Text>
              </Section>
              <Section style={totalItem}>
                <Text style={totalNumber}>{totalProposals}</Text>
                <Text style={totalLabel}>📄 Propostas</Text>
              </Section>
              <Section style={totalItem}>
                <Text style={totalNumber}>{totalDeals}</Text>
                <Text style={totalLabel}>🤝 Deals</Text>
              </Section>
            </Section>
          </Section>
          ...
          {operators.map((op, i) => (
            <Section key={i} style={operatorCard}>
              ...
              <Section style={statsRow}>
                <Text style={statItem}>
                  <span style={statIcon}>↗️</span> Avanços: <strong>{op.stageChanges}</strong>
                </Text>
              </Section>
              <Section style={statsRow}>
                <Text style={statItem}>
                  <span style={statIcon}>📅</span> Agendamentos: <strong>{op.appointments}</strong>
                </Text>
              </Section>
              <Section style={statsRow}>
                <Text style={statItem}>
                  <span style={statIcon}>📄</span> Propostas: <strong>{op.proposals}</strong>
                </Text>
              </Section>
              <Section style={statsRow}>
                <Text style={statItem}>
                  <span style={statIcon}>🤝</span> Deals: <strong>{op.deals}</strong>
                </Text>
              </Section>
            </Section>
          ))}
          ...
          {operators.length === 0 && (
            <Section style={emptyState}>
              <Text style={emptyText}>Nenhuma atividade registrada hoje.</Text>
            </Section>
          )}
          ...
          <Section style={whatsappSection}>
            ...
            <Section style={whatsappBlock}>
              <Text style={whatsappContent}>{whatsappText || 'Sem dados'}</Text>
            </Section>
          </Section>

          <Hr style={hr} />
          <Text style={footer}>{SITE_NAME} — Relatório de performance automático</Text>
        </Container>
      </Body>
    </Html>
  )
