...
interface CallScheduledAlertOverrides {
  title?: string
  subtitle?: string
  cta_text?: string
  no_briefing_warning?: string
  footer?: string
}
...
interface CallScheduledAlertProps {
  ...
  leadId?: string
  overrides?: CallScheduledAlertOverrides
}
...
const CallScheduledAlertEmail = ({
  ...
  const titleText = overrides.title || 'Nova proposta solicitada'
  const subtitleText = overrides.subtitle || 'Um lead avançou para <strong style="color:#5F2558">Call Agendada</strong>. Você tem <strong style="color:#EB626D">2 dias úteis</strong> para elaborar a proposta.'
  const ctaText = overrides.cta_text || '📄 Elaborar Proposta'
  const noBriefingWarning = overrides.no_briefing_warning || '⚠️ Sem briefing preenchido ainda. Preencha antes de elaborar a proposta.'
  const footerText = overrides.footer || `${SITE_NAME} — Alerta automático de proposta`
      ...
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={cardContainer}>
            ...
            <Section style={dataRow}><Text style={dataLabel}>Nome</Text><Text style={dataValue}>{leadName || '—'}</Text></Section>
            <Section style={dataRow}><Text style={dataLabel}>Empresa</Text><Text style={dataValue}>{company || '—'}</Text></Section>
            <Section style={dataRow}><Text style={dataLabel}>Cargo</Text><Text style={dataValue}>{cargo || '—'}</Text></Section>
            <Section style={dataRow}><Text style={dataLabel}>Telefone</Text><Text style={dataValue}>{telefone || '—'}</Text></Section>
            <Section style={dataRow}><Text style={dataLabel}>Email</Text><Text style={dataValue}>{email || '—'}</Text></Section>
            {workEmail ? (
              <Section style={dataRow}><Text style={dataLabel}>Email profissional</Text><Text style={dataValue}>{workEmail}</Text></Section>
            ) : null}
          </Section>
          ...
          <Section style={briefingContainer}>
            ...
            {hasBriefing ? (
              ...
              <Section style={noBriefingBox}>
                <Text style={noBriefingTextStyle}>{noBriefingWarning}</Text>
              </Section>
            )}
          </Section>
...
export const template = {
  ...
  editableFields: {
    title: { label: 'Título', default: 'Nova proposta solicitada', placeholder: 'Nova proposta solicitada' },
    subtitle: { label: 'Subtítulo (suporta HTML)', default: 'Um lead avançou para <strong style="color:#5F2558">Call Agendada</strong>. Você tem <strong style="color:#EB626D">2 dias úteis</strong> para elaborar a proposta.', placeholder: 'Um lead avançou para Call Agendada...' },
    cta_text: { label: 'Texto do botão', default: '📄 Elaborar Proposta', placeholder: '📄 Elaborar Proposta' },
    no_briefing_warning: { label: 'Aviso sem briefing', default: '⚠️ Sem briefing preenchido ainda. Preencha antes de elaborar a proposta.', placeholder: '⚠️ Sem briefing preenchido...' },
    footer: { label: 'Rodapé', default: 'Muti CRM — Alerta automático de proposta', placeholder: 'Muti CRM — Alerta automático de proposta' },
  },
...
const briefingContainer = { backgroundColor: '#f0ecea', borderRadius: '8px', padding: '16px 20px', margin: '0 24px 24px' }
const briefingText = { fontSize: '13px', color: '#333', lineHeight: '1.6', margin: '0', whiteSpace: 'pre-wrap' as const }
const noBriefingBox = { backgroundColor: '#fffbeb', borderRadius: '6px', borderLeft: '4px solid #F4A736', padding: '12px 16px' }
const noBriefingTextStyle = { fontSize: '13px', color: '#92400e', margin: '0' }
